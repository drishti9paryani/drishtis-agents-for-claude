...
- **Security audit:** open any project in Claude Code and say
  `run tars-secure` (or just "security check").
- **Go-offline build:** type `/unagi-askfirst` followed by your task, answer
  the one batch of questions, say "go", and walk away.
- **Feature & UX ideas:** say `run rancho-ideas` (or "what should I add
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "echo 'DAILY TOOLKIT REMINDER: The user has three global tools and wants a daily nudge to use them. 1) tars-secure agent - full security audit that detects, explains simply, fixes, then mandatorily re-checks - suggest after new code, auth/API changes, or before deploys. 2) rancho-ideas agent - read-only product review that suggests missing features and UI/UX improvements as simple pointers - suggest when an app reaches a working state or the user wonders what to build next. 3) unagi-askfirst skill - batches every question and permission upfront with an ALL CLEAR message, time estimate and simple todo list so the user can go offline - suggest before any long multi-step build task. Once per session, at a natural moment, remind the user of whichever tool fits the current work. Mention once; do not nag.'",
            "timeout": 10
          }
        ]
      }
...
- `~/.claude/agents/tars-secure.md`
- `~/.claude/agents/rancho-ideas.md`
- `~/.claude/skills/unagi-askfirst/` (the folder)
